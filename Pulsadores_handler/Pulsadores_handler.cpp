...
void PulsadoresHandler::setResponseRoute(uint8_t targetType, const TARGETNS& targetNS){
    s_respTargetType = targetType; s_respTargetNS = targetNS; s_responseModeEnabled = true;
}
void PulsadoresHandler::clearResponseRoute(){
    s_responseModeEnabled = false; s_respTargetType = 0; s_respTargetNS = NS_ZERO;
}
bool PulsadoresHandler::isResponseRouteActive(){ return s_responseModeEnabled; }
...
void PulsadoresHandler::setGlobalFxNoInput(bool enable) { s_globalFxNoInput = enable; }
...
bool PulsadoresHandler::isButtonPressed(byte color) {
    for (int i = 0; i < FILAS; i++) {
        ...
        for (int j = 0; j < COLUMNAS; j++) {
            ...
            if (ledIdx >= 0 && !isButtonEnabled(ledIdx)) continue;
...
void PulsadoresHandler::resolveContext(ButtonContext& ctx) {
    ...
    ctx.isCommunicatorBroadcast = (ctx.currentFile == "Comunicador" && _isNSZero(communicatorActiveNS));
...
void PulsadoresHandler::procesarPulsadores() {
    ...
    for (int i = 0; i < FILAS; i++) {
        ...
        for (int j = 0; j < COLUMNAS; j++) {
            ...
            if (color != RELAY) {
                if (!lastState[i][j] && isPressed) pressTime[i][j] = millis();
                else if (lastState[i][j] && !isPressed) pressTime[i][j] = 0;
            }
            ...
            if (color == RELAY) curRelayState |= isPressed;
            if (color == BLUE)  curBlueState  |= isPressed;
        }
        digitalWrite(filas[i], HIGH);
    }
    ...
    if (!respMode && !inModesScreen && ctx.currentFile != "Fichas" && ctx.hasPulse) {
        // En modos PULSE, recalculamos la salida en cada ciclo (o al menos cuando cambia algo)
...
// ==========================================
void PulsadoresHandler::processButtonEvent(int i, int j, ButtonEventType event, const ButtonContext& ctx) {
    ...
    if (ctx.currentFile == "Comunicador") {
        ...
        if (!ctx.isCommunicatorBroadcast && !ctx.hasBasic && !ctx.hasAdvanced && ctx.hasRelay) {
...
// 4.1 Menú Cognitivo
void PulsadoresHandler::handleCognitiveMenu(int i, int j, ButtonEventType event) {
    byte color = pulsadorColor[i][j];
    if (color != BLUE && color != GREEN && color != YELLOW && color != RED && color != RELAY) return;
...
void PulsadoresHandler::handleAmbientes(byte buttonColor, ButtonEventType event) {
    ...
    if (!selectedStates[currentIndex]) return;

    uint8_t sendVal = (buttonColor == RELAY) ? BLACK : buttonColor;
    send_frame(frameMaker_SEND_PATTERN_NUM(DEFAULT_BOTONERA, BROADCAST, NS_ZERO, sendVal));
    delay(100);
...
// ==========================================
void PulsadoresHandler::handleRelayLogic(byte buttonColor, ButtonEventType event, const ButtonContext& ctx) {
    
    // A) Multi-Relé / Aromaterapia (Colores)
    if (buttonColor != RELAY) {
        if (!ctx.isMultiRelay && !ctx.isAromaterapia) return; // Seguridad
        if (event != BUTTON_PRESSED) return;

        int idx = -1;
        if (buttonColor == BLUE) idx = 0;
        else if (buttonColor == GREEN) idx = 1;
        else if (buttonColor == YELLOW) idx = 2;
        else if (buttonColor == RED) idx = 3;
        ...
        if (idx >= 0) {
            ...
            // Feedback Color + Flag
            send_frame(frameMaker_SEND_COLOR(DEFAULT_BOTONERA, ctx.targetType, ctx.targetNS, buttonColor));
            delay(20);
            send_frame(frameMaker_SEND_FLAG_BYTE(DEFAULT_BOTONERA, ctx.targetType, ctx.targetNS, relay_state));
        }
        return;
    }
    ...
    if (buttonColor == RELAY && ctx.hasRelay) {
        ...
        if (ctx.targetType == DEFAULT_DEVICE && _isNSZero(ctx.targetNS)) return;

        if (ctx.hasPulse) {
            // Momentáneo
            bool on = (event == BUTTON_PRESSED);
            send_frame(frameMaker_SEND_FLAG_BYTE(DEFAULT_BOTONERA, ctx.targetType, ctx.targetNS, on ? 0x01:0x00));
            RelayStateManager::set(ctx.targetNS, on);
        } else {
            // Latching (Toggle)
            if (event == BUTTON_PRESSED) {
                bool cur = RelayStateManager::get(ctx.targetNS);
                send_frame(frameMaker_SEND_FLAG_BYTE(DEFAULT_BOTONERA, ctx.targetType, ctx.targetNS, !cur ? 0x01:0x00));
                RelayStateManager::set(ctx.targetNS, !cur);
            }
        }
    }
...
// ==========================================
void PulsadoresHandler::handleColorLogic(byte buttonColor, ButtonEventType event, const ButtonContext& ctx) {
    
    // 3.8 Passive (Solo Blue)
    if (ctx.hasPassive && buttonColor != BLUE) return;

    // 3.9 Patterns
    if (ctx.hasPatterns) {
        if (event == BUTTON_PRESSED) {
            send_frame(frameMaker_SEND_PATTERN_NUM(DEFAULT_BOTONERA, ctx.targetType, ctx.targetNS, buttonColor));
        }
        return;
    }
    ...
    if (ctx.hasBasic && !ctx.hasAdvanced) {
        ...
        if (ctx.hasPulse) {
            ...
            // 3.2 Básico Latched
            if (event == BUTTON_PRESSED) applyBasicLatched(buttonColor, ctx);
        }
        ...
        if (ctx.hasPulse) {
            ...
            if (event == BUTTON_PRESSED) applyAdvancedPaletteNoPulse(buttonColor, ctx);
        }
    }
}

// --- 6.1 Básico Latched (3.2) ---
void PulsadoresHandler::applyBasicLatched(byte buttonColor, const ButtonContext& ctx) {
    send_frame(frameMaker_SEND_COLOR(DEFAULT_BOTONERA, ctx.targetType, ctx.targetNS, buttonColor));
}
...
void PulsadoresHandler::applyBasicPulse(const ButtonContext& ctx) {
    ...
    if (anyPressed) {
        if (winner != currentSent) {
            send_frame(frameMaker_SEND_COLOR(DEFAULT_BOTONERA, ctx.targetType, ctx.targetNS, winner));
            currentSent = winner;
        }
    } else {
...
void PulsadoresHandler::applyAdvancedPaletteNoPulse(byte buttonColor, const ButtonContext& ctx) {
    ...
    if (currentModeIndex != lastMode) { palette.clear(); lastMode = currentModeIndex; }
    ...
    // Calcular Salida
    byte output = BLACK;
    if (palette.empty()) output = BLACK;
    else if (palette.size() == 1) output = palette[0];
    else if (palette.size() == 2) {
        colorHandler.color_mix_handler(palette[0], palette[1], &output);
    }

    send_frame(frameMaker_SEND_COLOR(DEFAULT_BOTONERA, ctx.targetType, ctx.targetNS, output));
}
...
void PulsadoresHandler::applyAdvancedPalettePulse(const ButtonContext& ctx) {
    ...
    // Recolectar pulsados (preservando orden si fuera necesario, o simplemente set)
    // Para simplificar, recorremos y llenamos.
    std::vector<byte> currentPressed;
    int count = 0;
    // Usamos orden de pressTime para determinar cuáles son los "últimos 2" si hubiera más de 2 dedos (caso raro)
    // Pero la especificación dice: max 2 en paleta. Si pulsas 3, la paleta se renueva al 3º.
    // Como esto corre en Loop continuo, es complejo detectar el "evento" de 3º pulsación aquí.
    // Simplificación Robustas: Si hay >2 pulsados, tomamos los 2 con mayor pressTime.
    
    struct BtnInfo { byte col; unsigned long t; };
    ...
    std::sort(pressedList.begin(), pressedList.end(), [](const BtnInfo& a, const BtnInfo& b){
        return a.t < b.t;
    });
